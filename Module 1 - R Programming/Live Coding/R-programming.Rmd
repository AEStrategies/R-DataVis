...
Let's learn about a few useful built-in functions that we can use with vectors. A function will be in the form: name_of_function(input)
...
We want to convert the animal vector into information that an algorithm or equation can understand more easily. Meaning we want to begin to check how many categories (factor levels) are in our character vector. We can pass the vector through the **factor()** function like so to get this information:
...
These errors indicate that these variables are only defined inside the scope of the function. So, variables defined inside of a function are only defined (or redefined) inside of that function. However, variables assigned outside of the function are global variables, and the function will have access to them due to their scope.
